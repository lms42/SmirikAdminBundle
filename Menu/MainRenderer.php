...
use \Knp\Menu\Renderer\Renderer as Renderer;
use \Knp\Menu\Renderer\RendererInterface as RendererInterface;
...
class MainRenderer extends Renderer implements RendererInterface
{
    ...
    public function renderItem(ItemInterface $item, array $options = array())
    {
        ...
        // opening li tag
        if ($item->hasChildren() && 0 !== $options['depth'] && $item->getDisplayChildren()) {
          $html = $this->format('<li'.$this->renderHtmlAttributes($attributes).' class="dropdown" data-dropdown="dropdown">', 'li', $item->getLevel());
    ...
    public function renderLink(ItemInterface $item, array $options = array())
    {
        ...
        if ($item->getUri() && (!$item->isCurrent() || $options['currentAsLink'])) {
            ...
            $text = sprintf('<a href="#" %s>%s</a>', $this->renderHtmlAttributes($item->getLabelAttributes()), $this->escape($item->getLabel()));
        }
